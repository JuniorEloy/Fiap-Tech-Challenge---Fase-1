...
class FinalizacaoOSResponse(BaseModel):
    ...
    leadtime_full_minutos: int = Field(
        ...,
        description="Tempo total decorrido do veículo na oficina (abertura até conclusão)",
    )
    leadtime_ativo_minutos: int = Field(
        ...,
        description="Tempo líquido de manutenção física (excluindo tempo de espera de aprovação)",
    )

    # Campos Financeiros Calculados Dinamicamente
    valor_servicos: Decimal = Field(
        ..., description="Soma dos serviços prestados (mão de obra)"
    )
    valor_pecas: Decimal = Field(
        ..., description="Soma das peças e insumos de estoque aplicados"
    )
    valor_total: Decimal = Field(
        ..., description="Valor final faturado e consolidado para cobrança"
    )
