...
def criar_requisicao_mock(
    headers: dict = None, cookies: dict = None, client_host: str = None
) -> Request:
...
def test_get_user_rate_limit_key_com_token_bearer_valido(mock_decodificar):
    ...
    """
    mock_decodificar.return_value = {
        "sub": "018f3a5b-7c10-7000-8000-000000000001",
        "role": "GERENTE",
    }
...
def test_get_user_rate_limit_key_com_token_bearer_sem_sub(mock_decodificar):
    ...
    req = criar_requisicao_mock(
        headers={"Authorization": "Bearer token_incompleto"}, client_host="10.0.0.5"
    )
...
def test_get_user_rate_limit_key_com_cookie_refresh():
    ...
    req = criar_requisicao_mock(
        cookies={"refresh_token": refresh_token_fake}, client_host="10.0.0.10"
    )
